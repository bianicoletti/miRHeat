...
Este aplicativo é uma interface interativa do pacote **miRHeat**, que permite processar arquivos de interações miRNA–UTR usando a função `parse_file()`.

## Funcionalidades

- Carregar arquivos tabulares ou texto (.txt, .csv, .tsv, etc.)
- Aplicar **expressões regulares** (pattern) para filtrar linhas/colunas
- Detectar automaticamente o formato tabular (opção “Detectar formato automaticamente”)
- Visualizar os dados processados em uma tabela interativa
- Mensagens informativas sobre o processamento (sucesso, erro ou sem resultados)
